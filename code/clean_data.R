...
# file 'academic_titles.csv' generated from pdf 'acad-title-codes.pdf'
# csv file generated manually via Excel generating function of Adobe Acrobat
# from Excel, this was saved as a .csv file
# manually done because no available dataset not in pdf format;
# only way to get data in .csv format

if (!file.exists('../data/academic_titles.csv'))
{
  titles <- read.csv('../rawdata/academic_titles.csv',
                     stringsAsFactors = FALSE)
  titles <- titles[1:511, c(1, 4)]


  titles$Title[grep('PROF OF CLIN- FY', titles$Title)] <- 'PROF OF CLIN-FY'
  titles$Title[grep("RES PROF-MILLER INST -AY", titles$Title)] <-
    "RES PROF-MILLER INST-AY"
  titles$Title[grep("PROF EMERITUS \\(WOS\\)", titles$Title)] <-
    "PROF EMERITUS(WOS)"
  titles$Title[grep("LECT SOE-EMERITUS \\(WOS\\)", titles$Title)] <-
  ...
  # write interim. Some text problems from the pdf->txt process
  # still have to be fixed by hand.
...
if (!exists('titles'))
{
  titles <- read.csv('../data/academic_titles.csv', strip.white = T,
                     stringsAsFactors = F)
}

uctca_colnames <- c("Name", "Title", "Base", "Overtime", "Other", "Benefits",
                    "Subtotal", "Total", "Year", "Notes", "Agency")

# restricting total pay to > $1000, this removes a lot of unnamed/useless rows.
if (!file.exists('../data/uc2011.csv') || !exists('uc2011'))
{
  uc2011 <- read_csv("../rawdata/university_of_california_2011.csv",
                     col_names = uctca_colnames,
                     skip = 1) %>% filter(Total > 1000)
  # this is not too useful because the 2011 titles are different from the
  # 2012:2014 titles.
  uc2011 <- left_join(uc2011, titles,
                      by = 'Title') %>% mutate(Academic = !is.na(Category))
...
{
  uc2012 <- read_csv("../rawdata/university_of_california_2012.csv",
                     col_names = uctca_colnames,
                     skip = 1) %>% filter(Total > 1000)
  uc2012 <- left_join(uc2012, titles,
                      by = 'Title') %>% mutate(Academic = !is.na(Category))
...
{
  uc2013 <- read_csv("../rawdata/university_of_california_2013.csv",
                     col_names = uctca_colnames,
                     skip = 1) %>% filter(Total > 1000)
  # 2013 and 2014 returned problems with read.csv();
  # so we terminate them with prejudice
  attributes(uc2013)$problems <- NULL
  uc2013 <- left_join(uc2013, titles,
                      by = 'Title') %>% mutate(Academic = !is.na(Category))
...
{
  # for some reason this dataset has an empty column, so we delete it.
  # Sometimes however, it does not.
  # Because the file is so large, we are having trouble figuring out
  # where this originated.
  uc2014 <- read_csv("../rawdata/university_of_california_2014.csv",
                     col_names = uctca_colnames,
                     skip = 1) %>% filter(Total > 1000)
  attributes(uc2014)$problems <- NULL
  uc2014 <- left_join(uc2014, titles,
                      by = 'Title') %>% mutate(Academic = !is.na(Category))
